...
**Phase 1 — Milestone 1: sign-in, organizations, GitHub App install, connected repositories, verified webhooks, queued runs**, plus a **landing-page milestone** (public marketing homepage; not Milestone 2), followed by landing-page redesign **Phase 1 (Design Foundation)**, **Phase 2 (Navigation and Hero)**, and now **Phase 3 (Interactive Validation Console)** per `PHASE_01_DESIGN_FOUNDATION.md` / `PHASE_02_NAVIGATION_AND_HERO.md` / `PHASE_03_INTERACTIVE_VALIDATION_DEMO.md`. See `LANDING_PAGE_IMPLEMENTATION.md` for the original landing-page build. Stopped after Phase 3 per the brief's explicit "do not start Phase 4" instruction.
...
The landing page (and these redesign phases within it) is a self-contained detour and doesn't change the underlying product recommendation below. Per `PHASE_03_INTERACTIVE_VALIDATION_DEMO.md`'s stop condition and the user's explicit instruction, **Phase 4 of the landing-page redesign has not been started** and should not begin without a separate go-ahead. If/when it does, it should: finish migrating `EvidenceTabs`'s tab buttons off the missing-background bug noted in Known issues #10 (still not done — `ValidationDemo`, the other component that bug mentioned, no longer exists after D-015); and consider fixing the pre-existing `CodeDiff`/`EvidenceTabs` 360px overflow bug noted in Known issues #17, since `EvidenceTabs` will likely be in scope by then.
